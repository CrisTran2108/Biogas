...
am4core.ready(function () {
  ...
  var chart = am4core.create("Ichartdb", am4charts.XYChart);

  //listen data from server
  let angrychartdata = [];
  for (i = 1; i < 10; i++) {
    angrychartdata.push({
      "date": `2020-08-17 16:${i * 5}:00`,
      "I_ab": Math.round(200 * Math.random()),
      "I_bc": Math.round(200 * Math.random()),
      "I_ca": Math.round(200 * Math.random()),
    })
  }

  chart.data = angrychartdata;
  socket.on('ichartdb', (data) => {
    //console.log(data);
    chart.data = data;
